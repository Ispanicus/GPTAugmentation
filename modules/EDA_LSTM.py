    ...
    for n in ns:
        ...
        while not success:
            try:
                ...
                success=True
                f.write(f"Used batch_size: {batch_size}\n")
            except:
                z+=1
        ...
        f.write(f"cuda memory usage before delete: {torch.cuda.memory_allocated(device='cuda')}\n")
        del X,Y,model
        torch.cuda.empty_cache()
        f.write(f"cuda memory usage after delete: {torch.cuda.memory_allocated(device='cuda')}\n")
        ...
        for aug in augs:
            ...
            while not success:
                try:
                    ...
                    model.train_(X, Y, batch_size=batch_size)
                    success=True
                    f.write(f"Used batch_size: {batch_size} for n:{n}, aug:{aug}\n")
                except:
                    z+=1
            ...
            f.write(f"memory usage before delete: {torch.cuda.memory_allocated(device='cuda')}\n")
            del X,Y,model
            torch.cuda.empty_cache()
            f.write(f"memory usage after delete: {torch.cuda.memory_allocated(device='cuda')}\n")
