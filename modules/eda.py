...
for augs in [64,32,16,8,4]:
	for path in subset_file_paths.paths:
		*localPath, filename =  path.split("/")
		...
		os.system('python code/augment.py --alpha_sr=0.05 --alpha_rd=0.05 --alpha_ri=0.05 --alpha_rs=0.0 --output={3}/eda/augs_{0}_n_{1}.txt --num_aug={0} --input={2}'.format(augs, n, path, localPath))